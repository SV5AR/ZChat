...
async function hkdf(
  ...
  const key = await crypto.subtle.importKey("raw", ikm.buffer, "HKDF", false, [
    "deriveBits",
  ]);
...
export async function ensureIdentity() {
  const existing = await localEncryptedDB.getItem('id_pair')
  if (existing && existing.priv && existing.pub) {
    return { priv: b64ToBuf(existing.priv), pub: b64ToBuf(existing.pub) }
  }
  const priv = crypto.getRandomValues(new Uint8Array(32));
  const pub = x25519.getPublicKey(priv) as Uint8Array;
  await localEncryptedDB.setItem('id_pair', { priv: bufToB64(priv), pub: bufToB64(pub) })
  return { priv, pub };
}

export async function ensureKyberKeypair() {
  const existing = await localEncryptedDB.getItem('kyber_pair')
  if (existing && existing.pk && existing.sk) {
    return { pk: b64ToBuf(existing.pk), sk: b64ToBuf(existing.sk) }
  }
  const m = await kyber.generateKeypair();
  const pk = m.publicKey as Uint8Array;
  const sk = m.secretKey as Uint8Array;
  await localEncryptedDB.setItem('kyber_pair', { pk: bufToB64(pk), sk: bufToB64(sk) })
  return { pk, sk };
}

export async function unlockStoreWithPin(pin: string){
  return await localEncryptedDB.initEncryptedStore(pin)
}
...
export async function publishPrekey() {
  ...
  const envelope = {
    id: bufToB64(id.pub),
    x25519_pub: Array.from(id.pub),
    kyber_pk: Array.from(k.pk),
    ts: new Date().toISOString(),
  };
  try {
    await publishEnvelope("prekeys", envelope);
  } catch (e) {
    /* best-effort */
  }
}
...
export async function initiateSession(
  ...
  concat.set(xShared, 0);
  concat.set(kyberShared, xShared.length);
