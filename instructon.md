i already have an idea and let me tell you

tell me the most top-tier encryption for those data only related to the account owner? for example best algorithm to derive the private key from the phrases (then privaet key derive infinite keys for each ratchet and purpose), so username needs public key, messages needs public key, friends request needs public key, and so on, guide me with the best strict, read my rules below then tell me your best approach:
